...
class MXMNet(nn.Module):
    def __init__(self,
        ...
        """
        MXMNet model making embeddings for molecular graphs.
        Args:

            dim (int): Dimensionality of the embeddings
            n_layer (int): Number of message passing layers. Defaults to 4.
            cutoff (float): Cutoff distance for interatomic interactions. Defaults to 5.0.
            num_spherical (int, optional): Number of spherical harmonics. Defaults to 7.
            num_radial (int, optional): Number of radial basis functions. Defaults to 6.
            envelope_exponent (int, optional): Exponent in the smooth cutoff. Defaults to 5.
        ...
        self.embeddings =  nn.Embedding(95, self.dim)
    
        self.rbf_l = BesselBasisLayer(16, 5, self.envelope_exponent)
        self.rbf_g = BesselBasisLayer(16, self.cutoff, self.envelope_exponent)
        self.sbf = SphericalBasisLayer(self.num_spherical, self.num_radial, 5, self.envelope_exponent)

        self.rbf_g_mlp = MLP([16, self.dim])
        self.rbf_l_mlp = MLP([16, self.dim])

        self.sbf_1_mlp = MLP([self.num_spherical * self.num_radial, self.dim])
        self.sbf_2_mlp = MLP([self.num_spherical * self.num_radial, self.dim])
    ...
    def forward(self, data):
        ...
        # Readout
        output = global_add_pool(h, batch)
        return output